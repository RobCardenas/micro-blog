$(function() {
    ...
    function UserPost(author,location,postTitle,userPost) {
        this.author = author;
    ...
    var testerPost = new UserPost("Rob", "Killafornia", "Conquering Yosemite's Half Dome","Did you know that anyone who's in shape and willing to suffer a whole day can climb Yosemite's Half Dome? Well, you can. First of all, you'll need a permit which you can get at Recreation.gov and pay a reservation fee if you're able to score a permit. You will not be able to go up the cables without a permit, I repeat, the ranger will deny you access to the cables if you don't have a permit.  ");
    ...
    $newPostSub.on("submit", function(event) {
        ...
        var authorLocation = $('#author-location').val();
        var newPostTitle = $('#new-post-title').val();
        var newPostContent = $('#new-post-content').val();
        var newPost = new UserPost(newAuthor,authorLocation,newPostTitle,newPostContent);

        if (newAuthor === '' || authorLocation === '' || newPostTitle === '' || newPostContent === '' ) {
            ...
            $posts.attr('data-index', newPost.author, newPost.location, newPost.postTitle, newPost.userPostContent);
            $postList.append($posts);
        }
        ...
        // Post Counter
        var $postCount = 0;
        if ( UserPost.allPosts.length === [] ) {
            $postCount;
        } else {
            $postCount += UserPost.allPosts.length;
        }
